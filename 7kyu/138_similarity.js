...
Get common items in both arrays
Return unique items length / common items length
*/

function similarity(a, b) {
	// const items = [...new Set([...a, ...b])];
	const items = new Set(a.concat(b));
	const commonItems = items.filter(num => a.includes(num) && b.includes(num));
	return commonItems.length / items.length;
}

console.log(similarity([1, 2, 3], [1, 2, 3])); //,1)

console.log(similarity([1, 2, 3], [4, 5, 6])); //,0)

console.log(similarity([1, 2, 4, 6, 7], [2, 3, 4, 7])); //,0.5)

console.log(similarity([1, 2, 6, 8, 9], [0, 1, 4, 5, 6, 8, 9])); //,0.5)

console.log(similarity([1, 2, 3, 4, 7, 9], [1, 2, 3])); //,0.5)

...
*/
/*
define a rates object with proximity as keys, rates as values
loop through array, adding (hours * correct rate) to a total
return total
*/
function bearDollars(arr) {
	...
	let total = 0;
	arr.forEach(item => {
		const [hours, proximity] = item;
		const calc = rates[proximity.toLowerCase()];
		const rate = typeof calc === 'number' ? calc : 125;
		total += hours * rate;
	});
	return total;
}

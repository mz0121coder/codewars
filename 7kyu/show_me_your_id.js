...
// Each word should be capitalised, (first letter uppercase, rest lowercase)
// String can be one word, or multiple words linked by '-'

const showMe = yourID => /^[A-Z][a-z]+(-[A-Z][a-z]+)*$/.test(yourID);

console.log(showMe('Francis')); //, true, "Francis is a name !")
console.log(showMe('Jean-Eluard')); //, true, "Jean-Eluard is a name!")
console.log(showMe('Le Mec')); //, false, "Le Mec is not a name!")
console.log(showMe('Bernard-Henry-Levy')); //, true, "Bernard-Henry-Levy is a name!")
console.log(showMe('Mémé Gertrude')); //, false, "Mémé Gertrude is not a name! It's my Grandma ")

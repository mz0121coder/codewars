...
// start with empty array for drawn cards
// make copy of deck
// while drawn cards length < deck length
// remove first card from copy, add to drawn cards
// move next card to bottom of copy
// repeat until drawn cards length === deck length, return drawn cards

const draw = deck => {
	const drawnCards = [];
	let cards = [...deck];
	while (drawnCards.length < deck.length) {
		const firstCard = cards.splice(0, 1);

...
Start avgWords, avgSyllables and totalWords at 0
Iterate through sentences:
  add words in line to totalWords
  add syllables in line to avgSyllables

avgWords = totalWords / number of sentences
divide avgSyllables by totalWords 

Use the formula to get the result to 2 d.p.
*/

function fleschKincaid(text) {
	const sentences = text.split(/[\?\.\!]/).filter(line => line.length > 0);
	let avgWords = 0;
	let avgSyllables = 0;
	let totalWords = 0;
	...
	sentences.forEach(line => {
		...
		const syllables = line.match(/[aeiou]+/gi);
		avgSyllables += syllables.length;
	});

	avgWords = totalWords / sentences.length;
	avgSyllables = avgSyllables / totalWords;

	return Number((0.39 * avgWords + 11.8 * avgSyllables - 15.59).toFixed(2));
}

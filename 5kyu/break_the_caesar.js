/*
The Caesar cipher is a notorious (and notoriously simple) algorithm for encrypting a message: each letter is shifted a certain constant number of places in the alphabet. For example, applying a shift of 5 to the string "Hello, world!" yields "Mjqqt, btwqi!", which is jibberish.

In this kata, your task is to decrypt Caesar-encrypted messages using nothing but your wits, your computer, and a set of the 1000 (plus a few) most common words in English in lowercase (made available to you as a preloaded variable named WORDS, which you may use in your code as if you had defined it yourself).

Given a message, your function must return the most likely shift value as an integer.
A few hints:

Be wary of punctuation
Shift values may not be higher than 25
*/

/*
repeat alphabet once to handle shifts up to 25
loop from 1 to 25:
extract words, replace non-letters, shift letters by current number
if all words are in the preloaded WORDS set, return shift
*/

function breakCaesar(st) {
	const alphabet = 'abcdefghijklmnopqrstuvwxyz'.repeat(2);
	for (let shift = 1; shift <= 25; shift++) {
		const words = st.split(' ').map(word =>
			word.replace(/[^a-z]/gi, '').replace(/[a-z]/gi, char => {
				const idx = alphabet.lastIndexOf(char.toLowerCase()) - shift;
				return alphabet[idx];
			})
		);
		if (words.every(word => WORDS.has(word))) return shift;
	}
}

const WORDS = new Set([
	'wtf',
	'cause',
	'control',
	'metal',
	'son',
	'inch',
	'rose',
	'six',
	'ran',
	'area',
	'tiny',
	'solution',
	'always',
	'black',
	'interest',
	'cook',
	'clock',
	'ease',
	'event',
	'leave',
	'post',
	'sound',
	'your',
	'learn',
	'major',
	'want',
	'card',
	'went',
	'bread',
	'plant',
	'men',
	'good',
	'brother',
	'white',
	'little',
	'great',
	'slave',
	'round',
	'rub',
	'both',
	'case',
	'short',
	'head',
	'pair',
	'collect',
	'several',
	'sentence',
	'near',
	'lie',
	'full',
	'will',
	'way',
	'bring',
	'wheel',
	'lay',
	'industry',
	'leg',
	'group',
	'those',
	'age',
	'prove',
	'century',
	'hear',
	'fresh',
	'shop',
	'lead',
	'gun',
	'energy',
	'describe',
	'week',
	'opposite',
	'law',
	'nation',
	'dead',
	'country',
	'young',
	'done',
	'remember',
	'air',
	'direct',
	'die',
	'shout',
	'test',
	'miss',
	'sun',
	'stay',
	'deal',
	'pretty',
	'tire',
	'populate',
	'well',
	'print',
	'check',
	'all',
	'one',
	'pass',
	'new',
	'would',
	'bank',
	'speed',
	'common',
	'am',
	'heard',
	'red',
	'late',
	'people',
	'try',
	'slip',
	'hand',
	'door',
	'push',
	'it',
	'ask',
	'rain',
	'liquid',
	'children',
	'get',
	'love',
	'else',
	'what',
	'fight',
	'old',
	'wear',
	'electric',
	'chart',
	'pose',
	'yet',
	'like',
	'seed',
	'surface',
	'since',
	'row',
	'low',
	'nor',
	'got',
	'through',
	'rope',
	'about',
	'our',
	'multiply',
	'force',
	'over',
	'differ',
	'must',
	'again',
	'told',
	'type',
	'press',
	'ground',
	'stop',
	'stood',
	'egg',
	'shoe',
	'observe',
	'insect',
	'string',
	'charge',
	'syllable',
	'glass',
	'only',
	'reach',
	'sat',
	'corner',
	'shine',
	'than',
	'support',
	'natural',
	'whose',
	'money',
	'segment',
	'man',
	'suit',
	'sugar',
	'ready',
	'fun',
	'rock',
	'quotient',
	'square',
	'by',
	'require',
	'except',
	'column',
	'self',
	'pull',
	'some',
	'tie',
	'wish',
	'especially',
	'follow',
	'noon',
	'fire',
	'claim',
	'branch',
	'certain',
	'a',
	'gave',
	'dress',
	'valley',
	'machine',
	'east',
	'kill',
	'come',
	'either',
	'board',
	'edge',
	'any',
	'process',
	'mile',
	'behind',
	'kind',
	'clothe',
	'stand',
	'pound',
	'pay',
	'place',
	'often',
	'base',
	'wonder',
	'total',
	'steam',
	'gas',
	'lost',
	'separate',
	'paper',
	'lake',
	'block',
	'the',
	'family',
	'every',
	'never',
	'strong',
	'see',
	'king',
	'root',
	'design',
	'thus',
	'sell',
	'an',
	'under',
	'gray',
	'bar',
	'feel',
	'for',
	'motion',
	'them',
	'thousand',
	'double',
	'how',
	'play',
	'no',
	'drink',
	'talk',
	'there',
	'enemy',
	'ride',
	'question',
	'should',
	'wing',
	'product',
	'where',
	'grow',
	'tube',
	'meat',
	'body',
	'protect',
	'radio',
	'class',
	'cent',
	'mix',
	'thick',
	'best',
	'thank',
	'when',
	'yellow',
	'melody',
	'cow',
	'chair',
	'stead',
	'hello',
	'heart',
	'find',
	'course',
	'job',
	'rest',
	'history',
	'yard',
	'captain',
	'out',
	'anger',
	'band',
	'travel',
	'foot',
	'copy',
	'mountain',
	'stick',
	'call',
	'while',
	'small',
	'bell',
	'tail',
	'blue',
	'been',
	'move',
	'experience',
	'is',
	'land',
	'sign',
	'roll',
	'select',
	'chord',
	'so',
	'continue',
	'bear',
	'much',
	'dry',
	'skin',
	'but',
	'noun',
	'wind',
	'food',
	'after',
	'station',
	'throw',
	'sky',
	'record',
	'oh',
	'free',
	'led',
	'middle',
	'kept',
	'fruit',
	'back',
	'pattern',
	'strange',
	'hundred',
	'still',
	'log',
	'count',
	'their',
	'human',
	'order',
	'favor',
	'him',
	'house',
	'path',
	'four',
	'cover',
	'box',
	'cost',
	'serve',
	'can',
	'long',
	'just',
	'nothing',
	'wrong',
	'produce',
	'locate',
	'level',
	'knew',
	'train',
	'whole',
	'swim',
	'and',
	'this',
	'language',
	'hunt',
	'reply',
	'they',
	'death',
	'watch',
	'figure',
	'wood',
	'quite',
	'time',
	'exact',
	'prepare',
	'possible',
	'master',
	'my',
	'of',
	'imagine',
	'dollar',
	'are',
	'found',
	'to',
	'if',
	'page',
	'compare',
	'act',
	'enter',
	'steel',
	'duck',
	'side',
	'seem',
	'hat',
	'word',
	'carry',
	'finger',
	'fair',
	'equal',
	'dad',
	'sure',
	'window',
	'fox',
	'weight',
	'team',
	'occur',
	'neighbor',
	'subtract',
	'think',
	'home',
	'able',
	'noise',
	'flat',
	'car',
	'mouth',
	'fall',
	'beauty',
	'silent',
	'center',
	'letter',
	'character',
	'lazy',
	'present',
	'very',
	'chance',
	'fly',
	'show',
	'tool',
	'wave',
	'represent',
	'range',
	'simple',
	'war',
	'agree',
	'gentle',
	'hold',
	'whether',
	'atom',
	'search',
	'offer',
	'winter',
	'who',
	'ball',
	'jump',
	'village',
	'clean',
	'season',
	'silver',
	'sudden',
	'arrive',
	'against',
	'fill',
	'wide',
	'wild',
	'gather',
	'felt',
	'cat',
	'choose',
	'plural',
	'together',
	'rule',
	'doctor',
	'general',
	'crop',
	'live',
	'life',
	'paint',
	'planet',
	'touch',
	'first',
	'milk',
	'salt',
	'practice',
	'farm',
	'spoke',
	'term',
	'condition',
	'green',
	'hit',
	'wait',
	'point',
	'less',
	'be',
	'face',
	'heavy',
	'stream',
	'skill',
	'spread',
	'nine',
	'stretch',
	'sit',
	'do',
	'wrote',
	'division',
	'person',
	'send',
	'turn',
	'fell',
	'go',
	'mark',
	'settle',
	'scale',
	'until',
	'friend',
	'parent',
	'plan',
	'climb',
	'written',
	'use',
	'final',
	'appear',
	'perhaps',
	'does',
	'child',
	'shore',
	'molecule',
	'thought',
	'material',
	'big',
	'answer',
	'rail',
	'hope',
	'most',
	'finish',
	'quick',
	'brown',
	'us',
	'develop',
	'close',
	'look',
	'horse',
	'third',
	'shape',
	'meant',
	'year',
	'floor',
	'mass',
	'cool',
	'hair',
	'proper',
	'grass',
	'result',
	'symbol',
	'share',
	'value',
	'could',
	'problem',
	'determine',
	'eat',
	'bad',
	'mind',
	'solve',
	'open',
	'which',
	'receive',
	'oxygen',
	'step',
	'day',
	'set',
	'cold',
	'tree',
	'notice',
	'special',
	'wire',
	'basic',
	'bit',
	'win',
	'effect',
	'between',
	'idea',
	'cross',
	'yes',
	'also',
	'guess',
	'poor',
	'vowel',
	'fear',
	'suggest',
	'region',
	'say',
	'lady',
	'property',
	'broke',
	'tall',
	'plane',
	'million',
	'company',
	'each',
	'hill',
	'his',
	'ship',
	'name',
	'number',
	'same',
	'dark',
	'sense',
	'method',
	'element',
	'match',
	'soldier',
	'port',
	'as',
	'animal',
	'dog',
	'month',
	'begin',
	'dictionary',
	'summer',
	'necessary',
	'shell',
	'student',
	'bright',
	'meet',
	'decimal',
	'danger',
	'forest',
	'teach',
	'supply',
	'he',
	'ago',
	'among',
	'sea',
	'right',
	'water',
	'safe',
	'put',
	'dream',
	'unit',
	'north',
	'eye',
	'made',
	'write',
	'left',
	'instant',
	'laugh',
	'surprise',
	'temperature',
	'picture',
	'know',
	'morning',
	'caught',
	'jumps',
	'before',
	'store',
	'rise',
	'then',
	'cloud',
	'particular',
	'usual',
	'more',
	'rich',
	'blood',
	'market',
	'current',
	'bottom',
	'star',
	'baby',
	'deep',
	'party',
	'table',
	'has',
	'trade',
	'we',
	'river',
	'fit',
	'afraid',
	'at',
	'woman',
	'began',
	'excite',
	'guide',
	'start',
	'crowd',
	'expect',
	'clear',
	'join',
	'difficult',
	'buy',
	'keep',
	'decide',
	'section',
	'enough',
	'earth',
	'science',
	'map',
	'line',
	'reason',
	'operate',
	'next',
	'famous',
	'substance',
	'busy',
	'subject',
	'here',
	'better',
	'might',
	'smile',
	'arm',
	'bat',
	'chick',
	'glad',
	'front',
	'few',
	'own',
	'oil',
	'slow',
	'system',
	'why',
	'connect',
	'original',
	'tell',
	'sing',
	'up',
	'with',
	'consonant',
	'govern',
	'bought',
	'she',
	'saw',
	'south',
	'brought',
	'similar',
	'bird',
	'gone',
	'create',
	'three',
	'least',
	'vary',
	'quiet',
	'were',
	'iron',
	'arrange',
	'nature',
	'early',
	'circle',
	'hole',
	'women',
	'pick',
	'desert',
	'study',
	'paragraph',
	'feed',
	'triangle',
	'book',
	'contain',
	'city',
	'from',
	'build',
	'length',
	'art',
	'break',
	'pitch',
	'soon',
	'dance',
	'cotton',
	'fish',
	'indicate',
	'space',
	'catch',
	'office',
	'allow',
	'feet',
	'key',
	'garden',
	'speak',
	'wife',
	'divide',
	'change',
	'mean',
	'view',
	'modern',
	'mine',
	'seat',
	'hot',
	'apple',
	'west',
	'five',
	'position',
	'bed',
	'period',
	'spot',
	'fast',
	'had',
	'warm',
	'beat',
	'score',
	'though',
	'matter',
	'moment',
	'camp',
	'success',
	'evening',
	'neck',
	'snow',
	'town',
	'lot',
	'speech',
	'sight',
	'school',
	'toward',
	'truck',
	'boy',
	'distant',
	'two',
	'seven',
	'island',
	'story',
	'ring',
	'off',
	'other',
	'walk',
	'corn',
	'thin',
	'road',
	'boat',
	'thing',
	'run',
	'sleep',
	'work',
	'above',
	'drive',
	'took',
	'did',
	'that',
	'was',
	'please',
	'heat',
	'sail',
	'sent',
	'size',
	'joy',
	'chief',
	'degree',
	'colony',
	'form',
	'take',
	'second',
	'fraction',
	'hurry',
	'single',
	'true',
	'trouble',
	'equate',
	'many',
	'end',
	'let',
	'far',
	'invent',
	'happen',
	'state',
	'raise',
	'listen',
	'quart',
	'half',
	'engine',
	'wash',
	'huge',
	'game',
	'include',
	'eight',
	'track',
	'crease',
	'dear',
	'room',
	'voice',
	'suffix',
	'list',
	'power',
	'loud',
	'phrase',
	'experiment',
	'bone',
	'provide',
	'poem',
	'said',
	'down',
	'now',
	'fat',
	'object',
	'world',
	'cut',
	'have',
	'draw',
	'help',
	'grand',
	'shoulder',
	'continent',
	'mount',
	'visit',
	'piece',
	'organ',
	'coat',
	'lift',
	'once',
	'straight',
	'wall',
	'street',
	'blow',
	'weather',
	'plain',
	'music',
	'soft',
	'complete',
	'believe',
	'burn',
	'instrument',
	'fact',
	'measure',
	'give',
	'twenty',
	'tone',
	'high',
	'me',
	'held',
	'came',
	'these',
	'read',
	'flow',
	'save',
	'works',
	'moon',
	'coast',
	'spring',
	'race',
	'fig',
	'large',
	'real',
	'correct',
	'repeat',
	'consider',
	'probable',
	'sand',
	'cry',
	'part',
	'ice',
	'forward',
	'color',
	'on',
	'capital',
	'happy',
	'cell',
	'nose',
	'smell',
	'shall',
	'girl',
	'make',
	'top',
	'stone',
	'grew',
	'such',
	'teeth',
	'or',
	'sheet',
	'born',
	'mother',
	'song',
	'sharp',
	'numeral',
	'need',
	'verb',
	'her',
	'rather',
	'too',
	'spell',
	'example',
	'care',
	'main',
	'depend',
	'during',
	'magnet',
	'may',
	'exercise',
	'trip',
	'discuss',
	'sister',
	'in',
	'field',
	'minute',
	'soil',
	'gold',
	'ocean',
	'ear',
	'add',
	'you',
	'night',
	'broad',
	'even',
	'last',
	'ten',
	'hour',
	'fine',
	'note',
	'past',
	'lone',
	'light',
	'spend',
	'ever',
	'flower',
	'hard',
	'father',
	'drop',
]);

console.log(breakCaesar('DAM?')); //, 7);
console.log(breakCaesar('Mjqqt, btwqi!')); //, 5);
console.log(breakCaesar('Gur dhvpx oebja sbk whzcf bire gur ynml qbt.')); //, 13);

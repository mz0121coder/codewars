...
// define variables to track scores for a and b
// store a and b in items array
// loop through items not equal to '*'
// on each iteration, start score at 0, match all elements:
// # or . (optional) followed by letters
// add 10000 to score for (id) to handle large test cases
// add 10 for classes
// add 1 for other elements
// add score to the variable for a if element in current iteration is a
// do the same for b
// return a if its score is higher, otherwise return b
...
function compare(a, b) {
	...
	items
		...
		.forEach(el => {
			let spec = 0;
			const elements = el.match(/(#|\.)?[a-z]+/g);
			elements.forEach(
				el =>
					(spec +=
						el === '*' ? 0 : el[0] === '#' ? 10000 : el[0] === '.' ? 10 : 1)
			);
			if (el === a) {
				aScore = spec;
			}

...
function peacefulYard(yard, minDistance) {
	const cats = [];
	// find grid points of cats
	for (let row = 0; row < yard.length; row++) {
		for (let col = 0; col < yard[row].length; col++) {
			if ('LMR'.includes(yard[row][col])) cats.push([row, col]);
		}
	}
	// calculate distances between cats
	for (let i = 0; i < cats.length; i++) {
		for (let j = i + 1; j < cats.length; j++) {
			const distance = Math.sqrt(
				Math.abs(Math.pow(cats[i][0] - cats[j][0], 2)) +
					Math.abs(Math.pow(cats[i][1] - cats[j][1], 2))
			);

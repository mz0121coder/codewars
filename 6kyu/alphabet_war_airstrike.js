...
/*
remove bombs and their adjacent letters
add up score for left and right side
...
function alphabetWar(fight) {
	let left = 0;
	let right = 0;
	const str = fight.replace(/[a-z]?\*+[a-z]?/g, '');
	for (const letter of str) {
		if ('sbpw'.includes(letter)) left += 'sbpw'.indexOf(letter) + 1;
		if ('zdqm'.includes(letter)) right += 'zdqm'.indexOf(letter) + 1;
	}
	console.log({ fight, str, left, right });
	return left > right
		? 'Left side wins!'
		: right > left
		? 'Right side wins!'

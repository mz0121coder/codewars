...
/*
Define 2 rows of alternating squares from 0 to dimension - 1, joined by ' '
row1 starts with light squares, row2 starts with dark squares

Loop from 0 to dimension - 1:
Update board, join and return
*/
...
function checkeredBoard(dimension) {
	...
	for (let i = 0; i < dimension; i++) {
		if (dimension % 2 === 0) {
			board.push(i % 2 === 0 ? row2 : row1);
		} else {
			board.push(i % 2 === 0 ? row1 : row2);
		}
	}
	return board.join('\n');
}

...
function getCard() {
	...
	const randomNum = (col, min, max) =>
		`${col}${Math.random() * (max - min) + min}`;

	for (let i = 1; i <= 5; i++) {
		B.push(randomNum('B', 1, 15));
		I.push(randomNum('I', 16, 30));
		if (i !== 3) N.push(randomNum('N', 31, 45));
		G.push(randomNum('G', 46, 60));
		O.push(randomNum('O', 61, 75));
	}
	cards.push(...B, ...I, ...N, ...G, ...O);
	return cards;
}

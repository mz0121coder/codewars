...
// calculate score
// push name and score to result array
// sort array from highest to lowest scores, map to show just the names

function scoring(array) {
	const result = [];
	array.forEach(el => {
		let score = 0;
		score += el.normKill * 100;
		score += el.assist * 50;
		score += el.damage * 0.5;
		score += el.healing;
		score += Math.pow(2, el.streak);
		score += el.envKill * 500;
		//   console.log({score})
		result.push({ name: el.name, score: score });
	});
	return result.sort((a, b) => b.score - a.score).map(el => el.name);
}

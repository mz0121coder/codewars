...
// return empty array if no devs with a null property
// otherwise find devs with a null property
// return array with those devs,
// add a question property with the message
// Hi, could you please prove your 'null property'

function askForMissingDetails(list) {
	const incomplete = list.filter(dev => Object.values(dev).includes(null));
	const questionAdded = incomplete.map(dev => {
		const nullProperty = Object.keys(dev).find(key => dev[key] === null);
		return {
			...dev,
			question: `Hi, could you please provide your ${nullProperty}.`,
		};
	});
	return !incomplete.length ? [] : questionAdded;
}

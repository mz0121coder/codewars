...
// sort addresses by state, store in an array
// sort addresses for each state by name of resident
// loop through each sorted state, check addresses ending in that state(sorted by names)
// add in correct format to result array
// join result array with '\r\n '

function byState(str) {
	const result = [];
	const states = ['AZ', 'CA', 'ID', 'IN', 'MA', 'OK', 'PA', 'VA'];
	const statesFull = [
		'Arizona',
		'California',
		'Idaho',
	...
	const addresses = str
		.split('\n')
		.sort((a, b) => states.indexOf(a.slice(-2)) - states.indexOf(b.slice(-2)))
		.map(address =>
			address
				.replaceAll(',', '')
				.replace(
					address.slice(-2),
					statesFull[states.indexOf(address.slice(-2))]
				)
		);

	statesFull.forEach(state => {
		const names = addresses.filter(item => item.endsWith(state)).sort();

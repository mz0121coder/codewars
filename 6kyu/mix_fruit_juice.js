...
// add 5 to sum for regular fruits
// add 7 for special ones
// add 9 for other fruits
// return sum / arr length, rounded to nearest integer
function mixFruit(arr) {
	...
	arr.forEach(item => {
		...
		if (regular.includes(itemName)) {
			sum += 5;
		} else if (special.includes(itemName)) {
			sum += 7;
		} else {
			sum += 9;
		}

/*Someone was hacking the score. Each student's record is given as an array The objects in the array are given again as an array of scores for each name and total score. ex>

var array = [
["name1", 445, ["B", "A", "A", "C", "A", "A"]],
["name2", 110, ["B", "A", "A", "A"]],
["name3", 200, ["B", "A", "A", "C"]],
["name4", 200, ["A", "A", "A", "A", "A", "A", "A"]]
];
The scores for each grade is:

A: 30 points
B: 20 points
C: 10 points
D: 5 points
Everything else: 0 points
If there are 5 or more courses and all courses has a grade of B or above, additional 20 points are awarded. After all the calculations, the total score should be capped at 200 points.

Returns the name of the hacked name as an array when scoring with this rule.

var array = [
["name1", 445, ["B", "A", "A", "C", "A", "A"]], // name1 total point is over 200 => hacked
["name2", 110, ["B", "A", "A", "A"]], //  name2 point is right
["name3", 200, ["B", "A", "A", "C"]], // name3 point is 200 but real point is 90 => hacked
,
["name4", 200, ["A", "A", "A", "A", "A", "A", "A"]] // name4 point is right
];

return ["name1", "name3"];
*/
// const result = []
// let score = 0
// loop over every item in array
// access nested array at index [i][2]
// add score based on letters and 20 point bonus for a nested array with 5 or more total letters and all letters equal to A or B
// if score !== arr[i][1], push arr[i][0] to result
function findHack(arr) {
	const result = [];
	for (let i = 0; i < arr.length; i++) {
		let score = 0;
		arr[i][2].forEach(letter => {
			switch (letter) {
				case 'A':
					score += 30;
					break;
				case 'B':
					score += 20;
					break;
				case 'C':
					score += 10;
					break;
				case 'D':
					score += 5;
			}
		});
		if (
			arr[i][2].length >= 5 &&
			arr[i][2].every(letter => letter === 'A' || letter === 'B')
		) {
			score += 20;
		}
		if (score !== arr[i][1]) {
			result.push(arr[i][0]);
		}
	}
	return result;
}

console.log(
	findHack([
		['name1', 150, ['B', 'A', 'A', 'C', 'A', 'A']],
		['name2', 120, ['B', 'A', 'A', 'A']],
		['name3', 160, ['B', 'A', 'A', 'A', 'A']],
		['name4', 140, ['B', 'A', 'A', 'C', 'A']],
	])
);

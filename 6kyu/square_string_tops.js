...
function tops(msg) {
	...
	while (true) {
		...
		const str = msg.slice(start, start + width);
		if (str.length) tops = str + tops;
		if (start >= msg.length || start + width >= msg.length) break;
	}
...
console.log(tops(''));
console.log(tops('abcde'));
console.log(tops('123456789abcdefghijklmnopqrstuwyxvzABCDEFGHIJKLMNOPQRSTU'));
console.log(
	tops(

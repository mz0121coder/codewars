...
// make a reusable function to count devs for any language
// check count for each language is less than or equal to 2 times the count for other languages

function isLanguageDiverse(list) {
	let isDiverse = true;
	const findLanguage = (l, lang) => l.filter(x => x.language === lang).length;
	const jsDevs = findLanguage(list, 'JavaScript');
	const pythonDevs = findLanguage(list, 'Python');
	const rubyDevs = findLanguage(list, 'Ruby');
	[jsDevs, pythonDevs, rubyDevs].forEach((el, i, arr) => {
		const otherLangs = [...arr.slice(0, i), ...arr.slice(i + 1)];
		if (otherLangs.some(x => x < el / 2)) {
			isDiverse = false;
		}
	});
	return isDiverse;

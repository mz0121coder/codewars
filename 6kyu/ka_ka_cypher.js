...
the way how we divide the words into "syllables" in the Kata.
 I don't want to make it too hard for other nations ;-P
*/
function kaCokadekaMe(word) {
	let cypher = [];
	for (let i = 0; i < word.length; i++) {
		if (word.substring(i, i + 2).match(/[aeiou][^aeiou]/gi)) {
			cypher.push(`${word[i]}ka`);
		} else {
			cypher.push(word[i]);
		}
	}
	return 'ka' + cypher.join('');
}

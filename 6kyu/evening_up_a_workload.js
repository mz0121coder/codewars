...
/*
start jim at 0 
start bob at sum of workload
start min diff at sum of workload

iterate through workload:
add current num to jim, subtract num from bob, check difference
reassign min diff and index of best split
*/

function splitWorkload(workload) {
	if (!workload.length) return [null, null];
	if (workload.length === 1) return [0, ...workload];

	let jim = 0;
	let bob = workload.reduce((acc, curr) => acc + curr, 0);
	let minDiff = Math.abs(jim - bob);
	let split = 0;
	let best = 0;

	for (const num of workload) {
		split++;
		jim += num;
		bob -= num;
		...
		if (minDiff === 0) return [best, 0];
	}

	return [best, minDiff];
}

console.log(splitWorkload([1, 6, 2, 3, 5, 4, 1])); //, [4, 2]

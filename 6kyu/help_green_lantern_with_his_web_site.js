...
function yellowBeGone(colorNameOrCode) {
	const str = colorNameOrCode;
	// handle hex codes
	if (/^#[a-z0-9]{6}$/i.test(str)) {
		const [red, green, blue] = [
			str.slice(1, 3),
			str.slice(3, 5),
			str.slice(5, 7),
		];
		const [r, g, b] = [red, green, blue].map(el => Number.parseInt(el, 16));
		// if red and green are both > blue
		if (r > b && g > b) {
			return r > g ? `#${blue}${red}${green}` : `#${blue}${green}${red}`;
		} else {
			return str;
		}
	}
	...
	for (const colour in obj) {
		if (str.toUpperCase() === colour.toUpperCase()) return obj[colour];
	}
	return str;
}

console.log(yellowBeGone('GOLD')); //, "ForestGreen")
console.log(yellowBeGone('pAlEgOlDeNrOd')); //, "LightGreen")
console.log(yellowBeGone('BlueViolet')); //, "BlueViolet")
console.log(yellowBeGone('#000000')); //, "#000000")
console.log(yellowBeGone('#b8860b')); //, "#0bb886")
console.log(yellowBeGone('#8FBC8F')); //, "#8FBC8F")
console.log(yellowBeGone('#C71585')); //, "#C71585")

...
const checkNum = el => typeof el === 'number' && el % 1 === 0;

Array.prototype.even = function () {
	return this.filter(item => checkNum(item) && item % 2 === 0);
};

Array.prototype.odd = function () {
	return this.filter(item => checkNum(item) && item % 2 === 1);
};

Array.prototype.under = function (x) {
	return this.filter(item => checkNum(item) && item < x);
};
...
Array.prototype.inRange = function (min, max) {
	return this.filter(item => checkNum(item) && item >= min && item <= max);
};

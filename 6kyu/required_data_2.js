...
function givenNthValue(arr, k, str_) {
	if (!arr.length) return 'No values in the array';
	if (k < 0) return 'Incorrect value for k';

	const direction = str_.toLowerCase();
	if (!/^(min|max)$/.test(direction)) return "Valid entries: 'max' or 'min'";

	const data = [...new Set(arr)];
	if (data.length < k) return 'No way';
	if (arr.some(el => typeof el !== 'number' || Number.isNaN(el)))
		return 'Invalid entry list';

	if (direction === 'min') return data.sort((a, b) => a - b)[k - 1];
	if (direction === 'max') return data.sort((a, b) => b - a)[k - 1];
}

let arr = [3, 3, -1, 10, 6, 8, -5, 4, 22, 31, 34, -16, -16, 8, 8]; // 15 elements
let k = 5;
let str_ = 'min';
console.log(givenNthValue(arr, k, str_)); //,4);

arr = [3, 3, -1, 10, 6, 8, -5, 4, 22, 31, 34, -16, -16, 8, 8];
k = 6;
str_ = 'max';
console.log(givenNthValue(arr, k, str_)); //,6);

arr = [3, 3, -1, 10, 6, 8, -5, 4, 22, 31, 34, -16, -16, 8, 8];
k = 13;
str_ = 'max';
console.log(givenNthValue(arr, k, str_)); //,"No way");

arr = [3, 3, -1, 10, 6, 8, -5, 'Yes', 4, 22, 31];
k = 4;
str_ = 'max';
console.log(givenNthValue(arr, k, str_)); //, "Invalid entry list");

arr = [];
k = 4;
str_ = 'max';
console.log(givenNthValue(arr, k, str_)); //,"No values in the array");

arr = [3, 3, -1, 10, 6, 8, -5, 4, 22, 31];
k = 2;
str_ = 'mix';
console.log(givenNthValue(arr, k, str_)); //,"Valid entries: 'max' or 'min'");

arr = [3, 3, -1, 10, 6, 8, -5, 4, 22, 31];
k = 2;
str_ = 'MaX';
console.log(givenNthValue(arr, k, str_)); //,22);

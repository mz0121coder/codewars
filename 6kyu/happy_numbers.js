...
// define a function squareDigits to sum of the squares of a number's digits
// call this function on initial number n, add to array
// if last array item is 1, return true
// define variable nextSquare which calls squareDigits on last array item
// if array already includes nextSquare, return false
// this is because it will cause an infinite loop, so n is not a happy number
// if array doesn't include nextSquare, push nextSquare to the array
...
function isHappy(n) {
	...
	const arr = [squareDigits(n)];
	while (true) {
		if (arr[arr.length - 1] === 1) {
			return true;
		}
		const nextSquare = squareDigits(arr[arr.length - 1]);
		if (arr.includes(nextSquare)) {
			return false;
		}
		if (!arr.includes(nextSquare)) {
			arr.push(nextSquare);

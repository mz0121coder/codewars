...
/*
define method that takes in a version string,
splits string by '.', first section has highest weighting,
each next section has 10% of the previous weighting,
add section score to a total and return total to show how recent the version is,
return recency(v1) >= recency(v2)
*/

function compareVersions(version1, version2) {
	const getRecency = str => {
		let divisor = 0.1;
		let total = 0;
		const sections = str.split('.');
		for (const item of sections) {
			divisor *= 10;
			total += Number(item) / divisor;
		}
		return total;
	};
	return getRecency(version1) >= getRecency(version2);
}
